    ...
    $.extend(window.Plugin.prototype, {
                    ...
                    if (data.method) {
                        ...
                        switch (data.method) {
                            ...
                            case 'error':
                                ...
                                break;
                            default:
                                alert('Unknown method: ' + data.method);
        ...
        saveToLocalStorage: function(data) {
            ...
            if (data.attributeDescription) {
                localStorage.setItem('pluginInitDataGeo', JSON.stringify(data.attributeDescription));
            }
        },
        ...
        pluginOpen: function(receivedData) {
            if (localStorage.getItem('pluginInitDataGeo')) {
                this._log(window.location.host + ' OPEN. GET DATA FROM LOCAL STORAGE', JSON.stringify(JSON.parse(localStorage.getItem('pluginInitDataGeo')), null, 4));
                $('.json__local-storage').text(JSON.stringify(JSON.parse(localStorage.getItem('pluginInitDataGeo')), null, 4));
            }
            // uncomment for degub
            $('.section__local-storage').show();
            $('.form').html(this.renderForm(receivedData));
            jsonCloseGeo.activity = $.extend(true, {}, receivedData.activity);

            _setLocalStorage('jsonCloseGeo', jsonCloseGeo);			
            ...
            $('#btnPosizione').click(function() {			
            	//debugger;

        	    var jsonTmp = JSON.parse(_getLocalStorage('jsonCloseGeo'));
              	console.log('before update position:' + JSON.stringify(jsonTmp));
        	    setGeoDataonJsonActivity(jsonTmp.activity);
        	    
        	    // jsonCloseGeoEmpty.activity.aid = jsonTmp.activity.aid;
        	    // //jsonCloseGeoEmpty.backActivityId =jsonTmp.activity.aid;
				// jsonCloseGeoEmpty.activity.position_in_route = jsonTmp.activity.position_in_route;                	
              	console.log('after update position:' + JSON.stringify(jsonTmp));
								
              	
              	this._sendPostMessageData(jsonTmp);
                  	                    
                _deleteLocalStorage('jsonCloseGeo');
            }.bind(this));
        ...
        getGeo: function(){
			...
			try {
				// try app js Interface
				callAndroid(jsonEventField);
			} catch (e) {
				// else try browser navigator API
				console.log("error calling android (falling back to browser): " + e.stack);
				geoFindMe(jsonEventField);
			}
